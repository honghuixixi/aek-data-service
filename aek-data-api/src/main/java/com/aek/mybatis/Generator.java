...
public class Generator {
	...
	public static void main(String[] args) {
		...
		gc.setOutputDir("D://MMMMMMMMMMMMMMMMMM");
		gc.setFileOverride(false);
		gc.setActiveRecord(false);
		gc.setEnableCache(false);// XML 二级缓存
		gc.setBaseResultMap(true);// XML ResultMap
		gc.setBaseColumnList(true);// XML columList
		gc.setAuthor("Honghui");
		...
		dsc.setUrl("jdbc:mysql://192.168.1.135:3306/aek_data_feat?characterEncoding=utf8");
		mpg.setDataSource(dsc);
		// 策略配置
		StrategyConfig strategy = new StrategyConfig();
//		strategy.setTablePrefix("sys_");// 此处可以修改为您的表前缀
		strategy.setNaming(NamingStrategy.underline_to_camel);// 表名生成策略
		//strategy.setInclude(new String[] { "cms_content" }); // 需要生成的表
		//strategy.setInclude(new String[] { "cms_reply_user" }); // 需要生成的表
