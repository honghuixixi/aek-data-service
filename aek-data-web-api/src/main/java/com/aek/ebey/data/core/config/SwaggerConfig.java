...
public class SwaggerConfig {
	...
	public Docket platformApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("ebey")
				...
				.select().apis(RequestHandlerSelectors.basePackage("com.aek.ebey.data"))
				.apis(RequestHandlerSelectors.withMethodAnnotation(ApiOperation.class)).paths(PathSelectors.any())
				.build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("data-web-api").description("©2017 Copyright. Powered By ebey.").build();
	}
